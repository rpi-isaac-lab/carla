...
class Waypoint_Finder(object):
	...
	def log_waypoints(self,filename,refresh_rate=240):
		# Refresh rate in Hz
		try:
			f = open(filename,"w+")
			f.write("time,id,roadid,laneid,s,Junction\n")
...
def print_nearest_waypoint(world):
	...
	nwp = map.get_waypoint(vehicle.get_location(),project_to_road=True,lane_type=(carla.LaneType.Driving))
	if nwp.is_junction:
	    print("WayPointID {}: RoadID=({:.02f}), LaneID=({:.02f}), Sdistance({:.02f}), JunctionID({:.02f}),".format(
		nwp.id,
		nwp.road_id,
		nwp.lane_id,
		nwp.s,
...
if __name__ == "__main__":
	...
	filename = "Junctionwaypoints.csv" #rename to desired filename
