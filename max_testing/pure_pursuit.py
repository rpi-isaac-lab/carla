...
class PurePursuit:
    ...
    def get_control(self, waypoints, speed):
        ...
        if self.obstacles_true:
            self.lane_shift = self.lane_change_eval(waypoints)
        # self.lane_shift = 0 #Uncomment for no obstacle avoidance
        ...
        if track_point is None:
            ...
            course_limit = np.abs(np.array((-193-t.location.y, 193.4-t.location.y, -74.4-t.location.x,231.75-t.location.x)))
            course_limit_index = np.where(course_limit == course_limit.min())[0]
            course_limit_signs = np.array((-193-t.location.y, 193.4-t.location.y, -74.4-t.location.x,231.75-t.location.x))/course_limit
            course_limit_directions = np.array((np.arctan2(-193,t.location.x+look_ahead_distance*2),np.arctan2(193.4,t.location.x-look_ahead_distance*2),np.arctan2(t.location.y-look_ahead_distance*2,-74.4),np.arctan2(t.location.y+look_ahead_distance*2,231.75)))
            
            print(course_limit_directions[course_limit_index])
            if np.abs(course_limit_directions[course_limit_index]) > .2:
                alpha = .2*course_limit_directions[course_limit_index]/np.abs(course_limit_directions[course_limit_index])
                steer = np.arctan((2 * self.wheel_base * np.sin(alpha)) / look_ahead_distance)
            else:
                alpha = course_limit_directions[course_limit_index]
                steer = np.arctan((2 * self.wheel_base * np.sin(alpha)) / look_ahead_distance)
            ...
            if waypoint27.is_junction:
                ...
                print("Override")
                steer = .1*course_limit_directions[course_limit_index]/np.abs(course_limit_directions[course_limit_index])
                
                return 0,0
            # return steer[0],alpha[0]
    ...
    def lane_change_eval(self,waypoints):
        ...
        for i, point in enumerate(self.blocked_points):
            ...
            if -15 < dis_from_object_x < 15 and -15 < dis_from_object_y < 15:
        ...
        f = open("TheaLogger.csv","w+")
        f.write(f_read)
        # waypoint27_transform = waypoint27.transform
        # string = str(waypoint27_transform.location.x)+","
        # string += str(waypoint27_transform.location.y)+","
        # string += str(waypoint27_transform.location.z)+","
        # string += str(waypoint27_transform.rotation.pitch)+","
        # string += str(waypoint27_transform.rotation.roll)+","
        # string += str(waypoint27_transform.rotation.yaw)+","
        # string += str(waypoint27.road_id)+","
        # string += str(waypoint27.section_id)+","
        # string += str(waypoint27.is_junction)+","
        # string += str(waypoint27.junction_id)+","
        # string += str(waypoint27.lane_id)+","
        # string += str(waypoint27.s)+","
        # string += str(waypoint27.lane_width)+"\n"
        # # print(waypoint27_transform)
        # f.write(string)
