...
# ==============================================================================
class Obstacles():
    def __init__(self,waypointroadids,waypointlaneids,waypointdistances,worldobject,worldcarla,simtime):
        ...
        self.spawn_index = self.waypointroadids.index(self.vehicle_spawn.road_id)  
    ...
    def simulation_file(self,object_type):
        ...
        if current_pos.distance(self.vehicle_spawn.transform.location) < 15.0:
            if self.lapcount<5:
                print(self.lapcount)
                self.lapcount += 1
                self.spawn_index = (self.spawn_index + 5) % len(self.waypointroadids)
                #self.reset(self.waypointroadids, self.waypointlaneids, self.waypointdistances, object_type, spawn_index)
                self.reset(object_type, self.spawn_index)
                
    def timer(self,time):
        if time-self.simtime>1:
            self.simtime=time
            return True
        else:
            return False
...
def game_loop(args):
    ...
    try:
        ...
        obstacles=Obstacles('/home/labstudent/carla/PythonAPI/max_testing/Data/ExactObjectWaypointsRoadIDs.csv','/home/labstudent/carla/PythonAPI/max_testing/Data/ExactObjectWaypointsLaneIDs.csv', '/home/labstudent/carla/PythonAPI/max_testing/Data/ExactObjectWaypointsS.csv',world,client.get_world(),hud.simulation_time)
