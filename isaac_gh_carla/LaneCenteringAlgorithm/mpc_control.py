# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 22:50:52 2025
...
class Controller2D(object):
    ...
    def update_controls(self, state):
        ...
        if self._start_control_loop:
            ...
            self.init_state.x = v * latency
            self.init_state.y = 0
            self.init_state.th = -v * self._set_steer * latency / 3
            self.init_state.v = v + (v - self.vars.v_previous)/ STEP_TIME * latency
            self.init_state.cte = cte + v * np.sin(yaw_err) * latency
            self.init_state.eth = yaw_err + self.init_state.th
            ...
            # print("[INFO] X: "+ str(self.init_state.x))
            # print("[INFO] Y: "+ str(self.init_state.y))
            print("[INFO] TH: "+ str(self.init_state.th))
            print("[INFO] V: "+ str(self.init_state.v))
            print("[INFO] CTE: "+ str(self.init_state.cte))
            print("[INFO] ETH: "+ str(self.init_state.eth))
            # print("[INFO] COFF: "+ str(coff))
            print("______________________________________________")
            #file = open('Errors.csv', 'a', newline='') 
            #writer = csv.writer(file)
            #writer.writerow([t, self.init_state.cte, self.init_state.eth])
...
class pursuitplusMPC:
    def __init__(self,waypoints,state=state()):
    
        self.control=Controller2D(waypoints)
        self.init_state=state
        
    def get_control(self,waypoints,speed,desired_speed,dt):
        self.control.update_controls(self.init_state)
        throttle=self.control._set_throttle
        steer=self.control._set_steer
        brake=self.control._set_brake
